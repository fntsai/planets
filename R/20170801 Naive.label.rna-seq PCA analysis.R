...
setwd("/Users/FuNien/Documents/2016_RNA_seq/20170415_POP3/Naive/3")
...
my.work.path <- "/Users/FuNien/Documents/2016_RNA_seq/20170415_POP3/Naive/3"
...
tmp.plot2 <- (qplot(PC2, PC3, data = dfPCA, color = condition,
                   main = pca_plot_title_2, size = I(3), 
                   label=rownames(dfPCA), geom=c("text", "auto"))
             + labs(x = paste0("PC2, Variance Explained: ", round(percentVar[1],4), "%"),
                    y = paste0("PC3, Variance Explained: ", round(percentVar[2],4), "%"))
             + scale_colour_manual(values=myPal)
             + theme(aspect.ratio = 1, 
                     panel.grid.minor = element_blank(),
                     panel.background = element_blank(), 
...
#create plot and output to .png file
CairoPDF(pca_plot_file_name_2, height=8, width=8, bg = "transparent")
(tmp.plot2)
dev.off()
